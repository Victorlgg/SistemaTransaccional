 ...
 * @author Victor
 */
@WebServlet(name = "ServletLimpiar", urlPatterns = {"/ServletLimpiar"})
public class ServletLimpiar extends HttpServlet {
...
//                boolean sw = Usuario.agregarUsuario(busuario);
                 ...
                 boolean sw = Cupos.limpiarCupos();
                if (sw) {
                    request.getRequestDispatcher("Mensajes.jsp").forward(request, response);
                } else {
                    PrintWriter out = response.getWriter();
                    out.println("Algo salió mal en la acción"
                            +"Conexión: "+c.getConexion()+" cupos limpiados:"+sw);
                }
...
//            out.println("<html>");
//            out.println("<head>");
//            out.println("<title>Servlet ServletLimpiar</title>");            
//            out.println("</head>");
//            out.println("<body>");
//            out.println("<h1>Servlet ServletLimpiar at " + request.getContextPath() + "</h1>");
//            out.println("</body>");
//            out.println("</html>");
//        }
    }
